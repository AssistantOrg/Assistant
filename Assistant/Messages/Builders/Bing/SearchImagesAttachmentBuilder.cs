...
{
    public class SearchImagesAttachmentBuilder : ContextBuilder<ImagesAttachment>
    {
        ...
        {
            var link = new Uri(_context.Options.BingLink + "/bing/v7.0/images/search");
            ...
            link = link.AddQuery("cc", _context.Options.Language);
            link = link.AddQuery("setLang", _context.Options.Language);

            return link;
        }

        private async Task<IEnumerable<ImageSearchInfo>> BuildAndSendHttpResuest()
        {
            ...
            client.DefaultRequestHeaders
                .Add("Ocp-Apim-Subscription-Key", _context.Options.BingToken);
            ...
            if (result.StatusCode == System.Net.HttpStatusCode.OK)
            {
                return value.Select(e => new ImageSearchInfo
                {
                    ...
                    SourceImage = new ImageInfo
                    {
                        ...
                        ExtensionSize = new Size((int)e["width"], (int)e["height"]),
                    },
                    ThumbnailImage = new ImageInfo
                    {
                        Link = new Uri((string)e["thumbnailUrl"]),
                        FileFormat = (string)e["encodingFormat"],
                        ExtensionSize = new Size((int)e["thumbnail"]["width"], (int)e["thumbnail"]["height"]),
                    }
                });
            }
            else
